"""
==========================================
版本：v3.0.12（2026-06-26）
相对 v3.0.11 改动：
  1) 修复金额提取将机器编号(12位)误当金额的严重bug（数电票PDF文本布局分散导致）
  2) 策略0/1/2窗口扩大：300/200/100 → 2000/200/200（适应数电票文本散乱）
  3) 新增策略1.5：从"价税合计"搜索到文档末尾找"(小写)¥"
  4) 策略3加固：要求金额必须有小数点(.XX格式)+长度≤10位
  5) 策略4加固：要求小数点+长度≤12位+排除机器编号区域数字
  6) 新增机器编号区域预处理：自动识别并排除"机器编号"后的长数字
相对 v3.0.10 改动：
  7) PDF文本提取多策略容错：fitz标准→fitz(dict)→fitz(rawdict)→pypdf
  8) 发票号启发式修正：AI返回19位时在连续0区域自动补0到20位
  9) 重试prompt强化：告诉AI差几位+连续0是最容易出错的地方
制作人：陆琦
==========================================
"""
...
def extract_invoice_no_from_pdf(pdf_bytes):
    ...
    m8 = re.search(r'(?<!\d)(\d{8})(?!\d)', clean)
    if m8:
        log.info(f"[{method}] PDF文本[通用正则]匹配独立8位: {m8.group(1)}")
        return m8.group(1)
    m20 = re.search(r'(?<!\d)(\d{20})(?!\d)', clean)
    if m20:
        log.info(f"[{method}] PDF文本[通用正则]匹配独立20位: {m20.group(1)}")
        return m20.group(1)
...
【发票号标准长度】8位（旧版）或20位（数电发票）
【发票号 vs 开户行账号/纳税人识别号】发票号在右上角"发票代码"下一行，不是购销方信息区的20位数字
【发票号"0"漏识别问题】数电发票号含连续多个0，必须逐位核对，不是8或20位就重新看
