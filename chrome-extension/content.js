...
function captureFile(file) {
  ...
  reader.onload = () => {
    ...
    if (icToggleVerify) {
      console.log('[发票检查 v3.0.47] 提交再检查模式：文件已缓存，等待用户点击【确定】后校验');
      return;
    }
    doCheck();
  };
...
function doCheck() {
  ...
  // v3.0.47: 【统一守卫】提交再查模式下，无论从哪条路径进入(doCheck被4处调用)，全部拦截
  ...
  if (icToggleVerify) {
    console.log('[发票检查 v3.0.47] doCheck 已拦截：提交再查模式开启中，即时识别暂停（文件已缓存，等点【确定】后校验）');
    return;
  }
...
function showFloat() {
  ...
  fw.innerHTML =
    '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">' +
      ...
      '<span style="font-weight:600;margin-right:4px">发票检查 v3.0.47</span>' +
  ...
  // 开关1 点击事件
  // v3.0.47: 互斥逻辑 — "提交再查"与即时识别两个开关互斥（同一时间只能选一种模式）
  ...
  fw.querySelector('#ic-toggle-verify-label').onclick = (e) => {
    ...
    if (icToggleVerify) {
      ...
      console.log('[发票检查 v3.0.47] 提交再查已开启 → 即时识别(发票号&购买方、金额&税率)已自动关闭');
    } else {
      unhookConfirmButton(); // 卸载拦截：恢复即时识别模式
      console.log('[发票检查 v3.0.47] 提交再查已关闭 → 恢复即时识别');
    }
    updateToggleUI(fw);
    // 切换开关后，安装或卸载确定按钮拦截器
    if (icToggleVerify) {
      hookConfirmButton(); // 安装拦截：监听确定按钮，收集表单数据发送校验
      console.log('[发票检查 v3.0.47] 提交再检查模式已开启：即时识别已停止，将监听【确定】按钮');
    } else {
      unhookConfirmButton(); // 卸载拦截：恢复即时识别模式
      console.log('[发票检查 v3.0.47] 提交再检查模式已关闭：恢复即时识别');
    }
  };
...
// v3.0.47: isConfirmBtnText 提升为全局函数（v3.0.47 定义在 hookConfirmButton 内部，
...
function hookConfirmButton() {
  ...
  // v3.0.47: 【关键修复】确定按钮可能在 .el-drawer 内，也可能在 .el-dialog 内
  ...
  // v3.0.47: 文字匹配改用 includes + 去除所有空白，兼容 Element UI 按钮文字含空格/换行的情况
  function bindConfirmBtn(btn) {
    ...
    btn.addEventListener('click', function onConfirmClick(e) {
      ...
      // v3.0.47: 不再调用 e.stopPropagation()，让 Element UI 原始保存 handler 正常执行
  ...
  let hookedCount = 0;
  // v3.0.47: 诊断增强 — 打印所有匹配按钮的实际文字，排查"找到4个但0个是确定"的问题
  const btnTexts = [];
  ...
  if (hookedCount > 0) {
    ...
    // v3.0.47: 没有命中"确定"时，进一步搜索页面上所有含"确定"文字的按钮
...
function unhookConfirmButton() {
  if (confirmBtnHandler && confirmBtnHandler._observer) {
    ...
    console.log('[发票检查 v3.0.47] 已卸载【确定】按钮拦截器');
  }
  // 移除所有已标记的 hook
...
function collectFormData() {
  ...
  console.log('[发票检查 v3.0.47] 收集到表单数据:', JSON.stringify(result));
  return result;
}
...
async function submitVerifyTask(fileName, fileData, formData) {
  ...
  try {
    ...
    console.log('[发票检查 v3.0.47] 校验完成:', taskId, result.error ? result.error : 'OK');

    // 更新 UI
    updateVerifyProgressUI();
    showNotify('校验完成', `「${fileName}」已校验完毕`);
  } catch (err) {
    console.error('[发票检查 v3.0.47] 校验请求失败:', err);
    const task = verifyQueue.find(t => t.id === taskId);
...
function updateVerifyProgressUI() {
  // v3.0.47: 浮标实际 id 是 'ic-float'（创建处在1378行），之前误写成 'ic-floating-widget' 导致 getElementById 返回 null、整函数直接 return
...
function saveVerifyReportsToStorage() {
  try {
    ...
    console.error('[发票检查 v3.0.47] 报告持久化失败:', e);
  }
}
  ...
  try {
    ...
    chrome.storage.local.get(['ic_verify_reports'], (res) => {
      ...
      if (saved && typeof saved === 'object' && Object.keys(saved).length > 0) {
        ...
        console.log('[发票检查 v3.0.47] 已从本地恢复', Object.keys(verifyReports).length, '份历史报告');
      }
    });
  } catch (e) {
    console.error('[发票检查 v3.0.47] 报告恢复失败:', e);
  }
}
...
console.log('[发票检查 v3.0.47] Content script已加载（时间戳保护 + 防重触发 + 白名单 + 发票号校验 + 三开关 + 重启图标 + 提交再查 + 报告持久化 + 自动注入 + 联调诊断console.log模式）');
