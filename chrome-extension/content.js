...
function captureFile(file) {
  ...
  reader.onload = () => {
    ...
    if (icToggleVerify) {
      console.log('[发票检查 v3.0.46] 提交再检查模式：文件已缓存，等待用户点击【确定】后校验');
      return;
    }
    doCheck();
  };
...
function doCheck() {
  ...
  // v3.0.46: 【统一守卫】提交再查模式下，无论从哪条路径进入(doCheck被4处调用)，全部拦截
  ...
  if (icToggleVerify) {
    console.log('[发票检查 v3.0.46] doCheck 已拦截：提交再查模式开启中，即时识别暂停（文件已缓存，等点【确定】后校验）');
    return;
  }
...
function showFloat() {
  ...
  fw.innerHTML =
    '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">' +
      ...
      '<span style="font-weight:600;margin-right:4px">发票检查 v3.0.46</span>' +
  ...
  // 开关1 点击事件
  // v3.0.46: 互斥逻辑 — "提交再查"与即时识别两个开关互斥（同一时间只能选一种模式）
  ...
  fw.querySelector('#ic-toggle-verify-label').onclick = (e) => {
    ...
    if (icToggleVerify) {
      ...
      console.log('[发票检查 v3.0.46] 提交再查已开启 → 即时识别(发票号&购买方、金额&税率)已自动关闭');
    } else {
      unhookConfirmButton(); // 卸载拦截：恢复即时识别模式
      console.log('[发票检查 v3.0.46] 提交再查已关闭 → 恢复即时识别');
    }
    updateToggleUI(fw);
    // 切换开关后，安装或卸载确定按钮拦截器
    if (icToggleVerify) {
      hookConfirmButton(); // 安装拦截：监听确定按钮，收集表单数据发送校验
      console.log('[发票检查 v3.0.46] 提交再检查模式已开启：即时识别已停止，将监听【确定】按钮');
    } else {
      unhookConfirmButton(); // 卸载拦截：恢复即时识别模式
      console.log('[发票检查 v3.0.46] 提交再检查模式已关闭：恢复即时识别');
    }
  };
...
// v3.0.46: isConfirmBtnText 提升为全局函数（v3.0.46 定义在 hookConfirmButton 内部，
//   导致自动诊断快照在全局作用域调用时 ReferenceError: isConfirmBtnText is not defined）
// 宽松匹配：去除所有空白后再比较，兼容 Element UI 按钮文字含空格/换行/零宽字符
function isConfirmBtnText(text) {
  return (text || '').replace(/\s+/g, '') === '确定';
}

function hookConfirmButton() {
  if (confirmBtnHandler) return; // 防止重复安装

  // v3.0.46: 【关键修复】确定按钮可能在 .el-drawer 内，也可能在 .el-dialog 内
  ...
  // v3.0.46: 文字匹配改用 includes + 去除所有空白，兼容 Element UI 按钮文字含空格/换行的情况
  function bindConfirmBtn(btn) {
    if (btn.dataset.icHooked === 'true') return; // 防止重复绑定
    btn.dataset.icHooked = 'true';
    // 诊断：打印按钮所在容器类型，便于联调确认 hook 是否命中正确按钮
    const container = btn.closest('.el-drawer, .el-dialog');
    const containerType = container ? (container.classList.contains('el-dialog') ? 'el-dialog' : 'el-drawer') : '未知';
    console.log('[发票检查] 已拦截【确定】按钮，容器类型=' + containerType);

    btn.addEventListener('click', function onConfirmClick(e) {
      // 条件检查 — 不满足时放行原始操作
      if (!icToggleVerify) {
        console.log('[发票检查] 点击【确定】但"提交再查"未开启，跳过校验（放行原保存）');
        return;
      }
      if (!lastCapturedFile) {
        showNotify('发票检查助手', '⚠️ 提交再查模式：请先上传发票附件');
        console.log('[发票检查] 点击【确定】但 lastCapturedFile 为空，已提示用户上传附件');
        return;
      }

      console.log('[发票检查] 【确定】被点击，icToggleVerify=' + icToggleVerify + '，lastCapturedFile=' + lastCapturedFile.fileName + '，开始收集表单并提交校验');
      // v3.0.46: 不再调用 e.stopPropagation()，让 Element UI 原始保存 handler 正常执行
  ...
  let hookedCount = 0;
  // v3.0.46: 诊断增强 — 打印所有匹配按钮的实际文字，排查"找到4个但0个是确定"的问题
  const btnTexts = [];
  existingBtns.forEach(btn => {
    const text = (btn.textContent || '').trim();
    btnTexts.push(text || '(空文字)');
    if (text === '确定') { bindConfirmBtn(btn); hookedCount++; }
  });
  console.log('[发票检查] hookConfirmButton 立即扫描：共找到', existingBtns.length, '个主按钮(selector=' + CONFIRM_BTN_SELECTOR + ')');
  console.log('[发票检查]   各按钮文字:', JSON.stringify(btnTexts));
  if (hookedCount > 0) {
    console.log('[发票检查]   其中【确定】', hookedCount, '个已绑定 ✅');
  } else {
    // v3.0.46: 没有命中"确定"时，进一步搜索页面上所有含"确定"文字的按钮
    const allBtnsWithText = Array.from(document.querySelectorAll('button, [role="button"], .el-button'))
      .filter(b => (b.textContent || '').includes('确定'))
      .map(b => ({
    ...
    if (allBtnsWithText.length > 0) {
      console.log('[发票检查]   ⚠️ 未用selector匹配到"确定"，但页面上的【确定】按钮长这样:');
      console.table(allBtnsWithText);
    } else {
      console.log('[发票检查]   ❌ 页面上完全没有含"确定"文字的按钮！可能Drawer/Dialog还没打开');
    }
  }

  // 步骤2：MutationObserver 监听后续 DOM 变化（先开开关后打开弹窗的场景）
  confirmBtnHandler = (mutations) => {
    for (const mut of mutations) {
      for (const node of mut.addedNodes) {
        if (!node.querySelectorAll) continue;
        const btns = node.querySelectorAll
          ? node.querySelectorAll(CONFIRM_BTN_SELECTOR)
          : [];
        for (const btn of btns) {
          if (isConfirmBtnText(btn.textContent)) bindConfirmBtn(btn);
        }
      }
    }
  };
...
function unhookConfirmButton() {
  if (confirmBtnHandler && confirmBtnHandler._observer) {
    ...
    console.log('[发票检查 v3.0.46] 已卸载【确定】按钮拦截器');
  }
  // 移除所有已标记的 hook
...
function collectFormData() {
  ...
  console.log('[发票检查 v3.0.46] 收集到表单数据:', JSON.stringify(result));
  return result;
}
...
async function submitVerifyTask(fileName, fileData, formData) {
  ...
  try {
    ...
    console.log('[发票检查 v3.0.46] 校验完成:', taskId, result.error ? result.error : 'OK');

    // 更新 UI
    updateVerifyProgressUI();
    showNotify('校验完成', `「${fileName}」已校验完毕`);
  } catch (err) {
    console.error('[发票检查 v3.0.46] 校验请求失败:', err);
    const task = verifyQueue.find(t => t.id === taskId);
...
function updateVerifyProgressUI() {
  // v3.0.46: 浮标实际 id 是 'ic-float'（创建处在1378行），之前误写成 'ic-floating-widget' 导致 getElementById 返回 null、整函数直接 return
...
function saveVerifyReportsToStorage() {
  try {
    ...
    console.error('[发票检查 v3.0.46] 报告持久化失败:', e);
  }
}
  ...
  try {
    ...
    chrome.storage.local.get(['ic_verify_reports'], (res) => {
      ...
      if (saved && typeof saved === 'object' && Object.keys(saved).length > 0) {
        ...
        console.log('[发票检查 v3.0.46] 已从本地恢复', Object.keys(verifyReports).length, '份历史报告');
      }
    });
  } catch (e) {
    console.error('[发票检查 v3.0.46] 报告恢复失败:', e);
  }
}
...
// ====== 诊断说明（v3.0.46）======
// content script 运行在 Chrome isolated world，与页面 JS 的 window 不互通。
// 所有诊断信息通过 console.log 输出（DevTools 会合并两个世界的日志）。
// 自动诊断快照在加载 2 秒后自动执行，打印浮标/开关/按钮/hook 状态。

console.log('[发票检查 v3.0.46] Content script已加载（时间戳保护 + 防重触发 + 白名单 + 发票号校验 + 三开关 + 重启图标 + 提交再查 + 报告持久化 + 自动注入 + 联调诊断console.log模式）');
...
setTimeout(() => {
  try {
    ...
    )).filter(b => isConfirmBtnText(b.textContent));
    ...
    console.log('%c[提示] 开【提交再查】开关后会输出更多诊断日志（各按钮文字、容器类型等）', 'color:#888');
  } catch(e) {
