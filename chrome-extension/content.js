...
function captureFile(file) {
  ...
  reader.onload = () => {
    ...
    if (icToggleVerify) {
      console.log('[发票检查 v3.0.40] 提交再检查模式：文件已缓存，等待用户点击【确定】后校验');
      return;
    }
    doCheck();
  };
...
function doCheck() {
  ...
  // v3.0.40: 【统一守卫】提交再查模式下，无论从哪条路径进入(doCheck被4处调用)，全部拦截
  ...
  if (icToggleVerify) {
    console.log('[发票检查 v3.0.40] doCheck 已拦截：提交再查模式开启中，即时识别暂停（文件已缓存，等点【确定】后校验）');
    return;
  }
...
function showFloat() {
  ...
  fw.innerHTML =
    '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">' +
      ...
      '<span style="font-weight:600;margin-right:4px">发票检查 v3.0.40</span>' +
  ...
  // 开关1 点击事件
  // v3.0.40: 互斥逻辑 — "提交再查"与即时识别两个开关互斥（同一时间只能选一种模式）
  ...
  fw.querySelector('#ic-toggle-verify-label').onclick = (e) => {
    ...
    if (icToggleVerify) {
      ...
      console.log('[发票检查 v3.0.40] 提交再查已开启 → 即时识别(发票号&购买方、金额&税率)已自动关闭');
    } else {
      unhookConfirmButton(); // 卸载拦截：恢复即时识别模式
      console.log('[发票检查 v3.0.40] 提交再查已关闭 → 恢复即时识别');
    }
    updateToggleUI(fw);
    // 切换开关后，安装或卸载确定按钮拦截器
    if (icToggleVerify) {
      hookConfirmButton(); // 安装拦截：监听确定按钮，收集表单数据发送校验
      console.log('[发票检查 v3.0.40] 提交再检查模式已开启：即时识别已停止，将监听【确定】按钮');
    } else {
      unhookConfirmButton(); // 卸载拦截：恢复即时识别模式
      console.log('[发票检查 v3.0.40] 提交再检查模式已关闭：恢复即时识别');
    }
  };
    ...
    for (const mut of mutations) {
      for (const node of mut.addedNodes) {
        ...
        for (const btn of btns) {
          ...
          console.log('[发票检查 v3.0.40] 已拦截【确定】按钮');

          // 使用事件捕获阶段，在原 click 之前执行校验提交
          btn.addEventListener('click', function onConfirmClick(e) {
            // v3.0.40: 条件检查 — 不满足时给明确提示（不再静默 return）
            if (!icToggleVerify) {
              console.log('[发票检查 v3.0.40] 【确定】被点击但"提交再查"未开启，跳过校验');
              // 仅日志记录，不打扰用户正常操作（未开启模式时不应该弹提示）
              return; // 未开启提交再查 → 放行原始操作
            }
            if (!lastCapturedFile) {
              console.log('[发票检查 v3.0.40] 【确定】被点击但没有已缓存的发票文件，跳过校验');
              // v3.0.40: 开启了提交再查但没上传文件 → 提醒用户
              showNotify('发票检查助手', '⚠️ 提交再查模式：请先上传发票附件');
              return;
            }

            console.log('[发票检查 v3.0.40] 【确定】被点击，开始收集表单数据并提交校验');
            e.stopPropagation();
...
function unhookConfirmButton() {
  if (confirmBtnHandler && confirmBtnHandler._observer) {
    ...
    console.log('[发票检查 v3.0.40] 已卸载【确定】按钮拦截器');
  }
  // 移除所有已标记的 hook
...
function collectFormData() {
  ...
  console.log('[发票检查 v3.0.40] 收集到表单数据:', JSON.stringify(result));
  return result;
}
...
async function submitVerifyTask(fileName, fileData, formData) {
  ...
  try {
    ...
    console.log('[发票检查 v3.0.40] 校验完成:', taskId, result.error ? result.error : 'OK');

    // 更新 UI
    updateVerifyProgressUI();
    showNotify('校验完成', `「${fileName}」已校验完毕`);
  } catch (err) {
    console.error('[发票检查 v3.0.40] 校验请求失败:', err);
    const task = verifyQueue.find(t => t.id === taskId);
...
function saveVerifyReportsToStorage() {
  try {
    ...
    console.error('[发票检查 v3.0.40] 报告持久化失败:', e);
  }
}
  ...
  try {
    ...
    chrome.storage.local.get(['ic_verify_reports'], (res) => {
      ...
      if (saved && typeof saved === 'object' && Object.keys(saved).length > 0) {
        ...
        console.log('[发票检查 v3.0.40] 已从本地恢复', Object.keys(verifyReports).length, '份历史报告');
      }
    });
  } catch (e) {
    console.error('[发票检查 v3.0.40] 报告恢复失败:', e);
  }
}
...
console.log('[发票检查 v3.0.40] Content script已加载（时间戳保护防Drawer重开误触发 + lastCompletedFile防取消重触发 + isInvoiceDrawer白名单 + 发票号手工校验 + 浮窗三开关 + 一键重启服务图标 + 提交再检查 + 弹窗关闭报告持久化keepalive/storage + 扩展重载自动注入兼容）');
