...
function showFloat() {
  ...
  fw.innerHTML =
    '<div style="display:flex;align-items:center;gap:8px;flex-wrap:wrap">' +
      ...
      '<span style="font-weight:600;margin-right:4px">发票检查 v3.0.35</span>' +
    ...
    '<div style="display:flex;align-items:center;gap:12px;margin-top:6px;font-size:11px">' +
      // 开关1: 发票号+购买方
      '<label style="display:flex;align-items:center;gap:5px;cursor:pointer;opacity:' + (icToggleBuyer ? '1' : '0.6') + '" id="ic-toggle-buyer-label">' +
        '<span>发票号&购买方</span>' +
        '<span id="ic-toggle-buyer" style="display:inline-block;width:32px;height:16px;border-radius:8px;background:' + (icToggleBuyer ? '#fff' : 'rgba(255,255,255,0.2)') + ';position:relative;transition:background .25s;flex-shrink:0">' +
          '<span style="position:absolute;top:2px;left:' + (icToggleBuyer ? '18px' : '2px') + ';width:12px;height:12px;border-radius:50%;background:' + (icToggleBuyer ? '#67c23a' : '#aaa') + ';transition:left .25s,background .25s"></span>' +
        '</span>' +
      '</label>' +
      // 开关2: 金额+税率
      '<label style="display:flex;align-items:center;gap:5px;cursor:pointer;opacity:' + (icToggleAmount ? '1' : '0.6') + '" id="ic-toggle-amount-label">' +
        '<span>金额&税率</span>' +
        '<span id="ic-toggle-amount" style="display:inline-block;width:32px;height:16px;border-radius:8px;background:' + (icToggleAmount ? '#fff' : 'rgba(255,255,255,0.2)') + ';position:relative;transition:background .25s;flex-shrink:0">' +
...
function updateToggleUI(fw) {
  ...
  buyerDot.style.left = icToggleBuyer ? '18px' : '2px';
  ...
  amountDot.style.left = icToggleAmount ? '18px' : '2px';
...
console.log('[发票检查 v3.0.35] Content script已加载（时间戳保护防Drawer重开误触发 + lastCompletedFile防取消重触发 + isInvoiceDrawer白名单 + 发票号手工校验 + 浮窗双开关 + 一键重启服务图标）');
